// concurrency_vs_parallelism.go - SOLUTION
...
// concurrencyThreshold defines the minimum slice size that justifies
// spawning goroutines. Below this, sequential sorting is faster because
// goroutine creation overhead exceeds the benefit of parallel execution.
...
// concurrentMergeSort only spawns goroutines when the slice is large
// enough for parallelism to pay off. Small slices are sorted sequentially.
func concurrentMergeSort(data []int) []int {
	...
	// Below threshold: sort sequentially to avoid goroutine overhead
	...
	// Above threshold: parallel execution is worthwhile
	var left, right []int
